...
def check_config(file_name):
    ...
    if args.scenario == '1':
        ...
        if file_name == BASIC_VPP_W_NRF or file_name == BASIC_W_NRF or file_name == BASIC_EBPF_W_NRF:
            ...
            cmd = f'{curl_cmd}"NSSF" | grep -o "192.168.70.139"'
            nssf_registration_nrf = run_cmd(cmd, False)
            if nssf_registration_nrf is not None:
                print(nssf_registration_nrf)
        else:
        ...
        if file_name == BASIC_VPP_W_NRF or file_name == BASIC_VPP_W_NRF_REDIRECT or file_name == BASIC_VPP_W_NRF_STEERING:
            ...
            cmd1 = 'docker logs oai-smf-slice1 2>&1 | grep "Received N4 ASSOCIATION SETUP RESPONSE from an UPF"'
            cmd2 = 'docker logs oai-smf-slice1 2>&1 | grep "Node ID Type FQDN: vpp-upf"'
            cmd3 = 'docker logs oai-smf-slice2 2>&1 | grep "Received N4 ASSOCIATION SETUP RESPONSE from an UPF"'
            cmd4 = 'docker logs oai-smf-slice2 2>&1 | grep "Node ID Type FQDN: vpp-upf"'

            upf_logs1 = run_cmd(cmd1)
            upf_logs2 = run_cmd(cmd2)
            upf_logs3 = run_cmd(cmd3)
            upf_logs4 = run_cmd(cmd4)
            if upf_logs1 is None or upf_logs2 is None:
                logging.error('\033[0;31m UPF slice 1 did not answer to N4 Association request from SMF\033[0m....')
                deployStatus = False
            if upf_logs3 is None or upf_logs4 is None:
                logging.error('\033[0;31m UPF slice 2 did not answer to N4 Association request from SMF\033[0m....')
                deployStatus = False
            else:
                logging.debug('\033[0;32m UPF did answer to N4 Association request from SMF\033[0m....')
            cmd1 = 'docker logs oai-smf-slice1 2>&1 | grep "PFCP HEARTBEAT PROCEDURE"'
            upf_logs1 = run_cmd(cmd1)
            if upf_logs1 is None:
                logging.error('\033[0;31m SMF slice 1 is NOT receiving heartbeats from UPF\033[0m....')
                deployStatus = False
            else:
                logging.debug('\033[0;32m SMF slice 2 is receiving heartbeats from UPF\033[0m....')
        elif file_name == BASIC_W_NRF:
            logging.debug('\033[0;34m Checking if SMF is able to connect with UPF\033[0m....')
            cmd1 = 'docker logs oai-smf-slice1 2>&1 | grep "Received N4 ASSOCIATION SETUP RESPONSE from an UPF"'
            cmd2 = 'docker logs oai-smf-slice1 2>&1 | grep "Resolve IP Addr 192.168.70.134, FQDN oai-upf-slice1"'
            cmd3 = 'docker logs oai-smf-slice2 2>&1 | grep "Received N4 ASSOCIATION SETUP RESPONSE from an UPF"'
            cmd4 = 'docker logs oai-smf-slice2 2>&1 | grep "Resolve IP Addr 192.168.70.140, FQDN oai-upf-slice2"'
            upf_logs1 = run_cmd(cmd1)
            upf_logs2 = run_cmd(cmd2)
            upf_logs3 = run_cmd(cmd3)
            upf_logs4 = run_cmd(cmd4)
            if upf_logs1 is None or upf_logs2 is None:
                logging.error('\033[0;31m UPF slice1 did not answer to N4 Association request from SMF\033[0m....')
                deployStatus = False
            if upf_logs3 is None or upf_logs4 is None:
                logging.error('\033[0;31m UPF slice 2 did not answer to N4 Association request from SMF\033[0m....')
                deployStatus = False
            ...
            upf_logs1 = run_cmd(cmd1)
            if upf_logs1 is None:
                logging.error('\033[0;31m SMF slice1  is NOT receiving heartbeats from UPF\033[0m....')
                deployStatus = False
            else:
                logging.debug('\033[0;32m SMF slice1 is receiving heartbeats from UPF\033[0m....')
            cmd1 = 'docker logs oai-smf-slice2 2>&1 | grep "PFCP HEARTBEAT PROCEDURE"'
            upf_logs1 = run_cmd(cmd1)
            if upf_logs1 is None:
                logging.error('\033[0;31m SMF slice2  is NOT receiving heartbeats from UPF\033[0m....')
                deployStatus = False
            else:
                logging.debug('\033[0;32m SMF slice2 is receiving heartbeats from UPF\033[0m....')
        else:
            logging.debug('\033[0;34m Checking if SMF is able to connect with UPF\033[0m....')
